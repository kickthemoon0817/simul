...
1. **Ask the user** for the absolute path to their `.uproject` (skip if
   unambiguous from CWD / conversation) and confirm before touching it. Ask
   for `--engine-path` only if auto-detection is likely to fail (custom UE
   build, non-standard install, Linux without `UnrealEditor` on `PATH`).
2. **Run `simul unreal setup <.uproject> --yes`.** It patches the
   `.uproject` and `Config/DefaultRemoteControl.ini`, launches the editor
   **headless** (pass `--no-headless` only when the user wants the GUI), and
   polls `unreal_health_check` until Remote Control answers. If the editor is
   already running, add `--no-launch`.
3. **Then use simul's Unreal tools** (`capture_unreal_viewport`,
   `execute_unreal_script`, …).

What setup writes, launcher detection per OS, and re-run behaviour are in
`docs/unreal-setup.md`.

### Hard rules

- Prefer `simul unreal setup` over hand-editing the `.uproject` or ini.
- In automated flows ask the user once, then pass `--yes`.
- Don't hardcode macOS paths on Linux or vice versa; `--engine-path`
  overrides detection. Windows has no automated launch — use `--no-launch`.
- If setup exits non-zero, surface the actual error (plugins not enabled,
  Python execution disabled, port conflict, editor crashed). Don't pretend it
  worked.

### Cross-host / remote access — only when the user actually needs it

`--bind`, `--allow-public`, `--passphrase` and `--websocket-port` widen the
trust radius; don't use them speculatively. Remote Control has no auth and
simul enables remote Python, so before any non-loopback `--bind` ask: *"Is
the network behind a firewall, or are you OK exposing arbitrary Python
execution to it?"* and pass `--allow-public` only after they confirm. Details
and examples: `docs/unreal-setup.md`.
...
(6.0+ only; turns on python_server `require_auth` with a Kit-generated token
that the MCP server reads from the discovery dir, so nothing secret reaches
argv — prefer it over `--auth-token <value>`, which needs a matching
`ISAAC_SIM__SOCKET_AUTH_TOKEN`), `--kit-arg` (repeatable passthrough),
`--log-file`. When the bridge ext isn't published under
...
- `pytest tests/` should be 100% green on `main`. The skips are the
  ...
  tests that need a running engine; the deselected tests are the slow
  `packaging` marker tests (`tests/packaging/`), which addopts skips by
  default — run them with `pytest -m packaging` (the last `-m` wins). New
  failures are real regressions; debug them.
- Coverage is on by default (`--cov` is in addopts). For the fast loop use
  `pytest tests/ --no-cov`.
