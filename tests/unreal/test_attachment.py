...
def settings(tmp_path):
    base = Settings()
    return base.model_copy(update={"unreal": base.unreal.model_copy(update={
        "mode": "attached", "attachment_path": str(tmp_path / "attachment.json"),
        "host": "127.0.0.1", "ping_timeout": 1,
    })})
...
def editor(monkeypatch):
    ...
    unreal = SimpleNamespace(
        LevelEditorSubsystem="level", UnrealEditorSubsystem="editor",
        get_editor_subsystem=lambda name: level if name == "level" else SimpleNamespace(get_editor_world=lambda: world),
        ...
        try:
            with redirect_stdout(output):
                if params["ExecutionMode"] == "EvaluateStatement":
                    ...
                    value = None
            return web.json_response({"ReturnValue": True, "CommandResult": repr(value),
                "LogOutput": [{"Type": "Info", "Output": line} for line in output.getvalue().splitlines()]})
        except Exception as exc:
            return web.json_response({"ReturnValue": False, "CommandResult": str(exc), "LogOutput": []})
...
@pytest.mark.asyncio
async def test_guards_execute_on_editor_before_scripts_and_legacy_mutations(settings, editor):
    ...
    try:
        ...
        result = await session._execute_python("import unreal; unreal.events.append('first')")
        assert result["ReturnValue"] is True
        assert events == ['first']
        assert (await session._execute_python("40 + 2", mode="EvaluateStatement"))["CommandResult"] == "42"
        assert (await session._execute_python("import unreal; unreal.events.append('statement')",
                                              mode="ExecuteStatement"))["ReturnValue"] is True
        ...
        for callback in callbacks:
            callback(1)
        result = await session._execute_python("import unreal; unreal.events.append('wrong map')")
        assert result["ReturnValue"] is False and 'attachment changed' in result["CommandResult"]
        before = len(calls)
...
async def test_attach_requires_unambiguous_editor_and_viewport(settings, monkeypatch):
    manager = UnrealAttachments(settings)
    info = {"instance_id": "one", "document_id": "doc", "project_path": "/project.uproject",
            "map_path": "/Game/Map", "host": "localhost", "port": 30010,
            "viewports": ["a", "b"], "reachable": True}
    ...
    with pytest.raises(ValueError, match="unique level viewport"):
        await manager.attach()
    monkeypatch.setattr(manager, 'describe', AsyncMock(return_value={**info, "document_id": "changed"}))
...
@pytest.mark.parametrize('kwargs', [
    ...
    {"agent_control": "set_property", "property_name": "__class__", "value": [1, 2, 3]},
    {"agent_control": "set_property", "property_name": "location", "value": [float('nan'), 0, 0]},
...
async def test_invalid_controls_fail_before_any_connection(settings, kwargs):
    session = UnrealRuntimeSession(settings)
    session._ensure_http_session = AsyncMock(side_effect=AssertionError('must not connect'))
