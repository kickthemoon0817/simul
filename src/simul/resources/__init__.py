"""Data files shipped inside the ``simul`` wheel.
...
def resource(*parts: str) -> Traversable:
    """Return the packaged data file at ``simul/resources/<parts...>``.
...
def find_checkout_root() -> Optional[Path]:
    ...
    Returns:
        The directory holding ``pyproject.toml`` and ``src/simul`` for an
        editable install, or ``None`` for a wheel install where the package
        sits in ``site-packages`` and no such root exists.
    """
    candidate = Path(__file__).resolve().parents[3]
    if (candidate / "pyproject.toml").is_file() and (candidate / "src" / "simul").is_dir():
        return candidate
    return None
