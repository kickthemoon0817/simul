...
from simul.adapters import IsaacSocketClient
...
from simul.adapters.isaac_runtime import IsaacRuntimeAdapter
...
def bridge_up(
    ...
    3. Else enable ``khemoo.simul`` via the VS Code fallback, then
       re-probe the bridge.
    ...
    if not vscode_ok:
        msg = (
            ...
            "enables neither at startup; launch it with `simul isaac launch` "
    ...
    enable_result = asyncio.run(
        tools.enable_isaac_extension(extension_id="khemoo.simul")
    )
    if not enable_result.get("success"):
        msg = (
            "VS Code transport reached but enabling khemoo.simul "
...
def launch(
    ...
    bridge_port: Optional[int] = typer.Option(
        None,
        "--bridge-port",
        help="Port for the khemoo.simul bridge. Defaults to isaac_sim.bridge_port (8229).",
    ),
    ...
    if not bridge_present:
        result["hint"] = (
            ...
            f"`ISAAC_SIM_PATH={root} simul isaac install-bridge --symlink` for the typed bridge."
        )
    if dry_run:
        result["success"] = True
        result["dry_run"] = True
...
def install_bridge(
    isaac_root: Optional[Path] = typer.Option(
        ...
        help=(
            ...
            "The bridge ext is published into <isaac-root>/extsUser/khemoo.simul/"
    ...
    source: Optional[Path] = typer.Option(
        ...
        help=(
            ...
            "<simul>/bridge_ext/khemoo.simul/, which ships in both "
    ...
    source ships inside the package at ``simul/bridge_ext/khemoo.simul/``.

    Workflow:

      1. Resolve isaac-root: --isaac-root flag → $ISAAC_SIM_PATH → error.
      2. Resolve source: --source flag → bundled
         ``simul/bridge_ext/khemoo.simul/`` → error.
    ...
    if source_p is None or not (source_p / "config" / "extension.toml").is_file():
        msg = (
            ...
            "should ship at simul/bridge_ext/khemoo.simul/ — if "
            "it's missing, the wheel may have been built without "
            "package-data; reinstall with `pip install -e .` from a "
            "repo checkout, or `pip install --force-reinstall simul-toolkit`."
        )
...
def bridge_config(
    ...
    script = textwrap.dedent(
        ...
        print(json.dumps({
            "extension_enabled": manager.is_extension_enabled("khemoo.simul"),
            "host": settings.get("/exts/khemoo.simul/host"),
            "port": int(settings.get("/exts/khemoo.simul/port")),
            "allow_unsafe_execution": bool(settings.get("/exts/khemoo.simul/allow_unsafe_execution")),
            "max_request_bytes": int(settings.get("/exts/khemoo.simul/max_request_bytes")),
            "max_response_bytes": int(settings.get("/exts/khemoo.simul/max_response_bytes")),
        }))
        """
    )
    try:
...
def bridge_set_unsafe(
    enable: bool = typer.Option(
        ...
        help=(
            ...
            "SECURITY__ALLOW_SCRIPT_EXECUTION=false on the simul server to "
    ...
    script = textwrap.dedent(
        ...
        settings = carb.settings.get_settings()
        settings.set("/exts/khemoo.simul/allow_unsafe_execution", {str(enable)})
        manager = omni.kit.app.get_app().get_extension_manager()
        extension_name = "khemoo.simul"
