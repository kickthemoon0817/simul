...
``import simul_mcp.cli.main`` took ~720 ms because ``simul_mcp.adapters``
...
_HEAVY = ("fastmcp", "pxr", "aiohttp", "bpy", "simul_mcp.mcp.server")
...
def test_cli_import_skips_server_and_runtimes() -> None:
    loaded = _modules_after("import simul_mcp.cli.main")
...
def test_adapters_package_resolves_exports_on_access() -> None:
    loaded = _modules_after(
        "import simul_mcp.adapters as a\n"
        "assert a.IsaacSocketClient.__name__ == 'IsaacSocketClient'\n"
        "assert callable(a.is_unreal_available)"
    )
    assert "simul_mcp.adapters.isaac_socket_client" in loaded
    assert "simul_mcp.adapters.headless_usd" not in loaded
    assert "simul_mcp.adapters.blender_runtime" not in loaded
